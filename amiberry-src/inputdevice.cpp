...
static void osk_control(int x, int y, int button, int buttonstate)
{
	...
	{
		// Android runtime VKBD is operated via touch/mouse pointer hit-testing.
		// The legacy osk_control() navigation path (axes/buttons -> directions)
		// can generate continuous directional events from noisy inputs and will
		// fight the pointer handler, causing "random" selection flashing and
		// preventing taps from producing stable press/release transitions.
...
static bool get_mouse_position(int *xp, int *yp, int inx, int iny)
{
...
#endif
	{
		...
		y = (int)(y * fmy);
		x -= (int)(fdx * 1.0) - 0;
		// The historical "- 2" fudge factor causes a constant Y offset that is very visible
		// on Android touch/mouse input (can't reach the very top). Keep legacy behavior on
		// desktop but avoid it on Android.
		#ifdef AMIBERRY_ANDROID
		y -= (int)(fdy * 1.0);
		#else
		y -= (int)(fdy * 1.0) - 2;
		...
		// x is already scaled by fmx above; don't multiply again or we clamp too early.
		if (x >= vidinfo->outbuffer->outwidth) {
			ob = true;
		...
		// y is already scaled by fmy above; don't multiply again or we clamp too early.
		if (y >= vidinfo->outbuffer->outheight) {
			ob = true;
			y = vidinfo->outbuffer->outheight - 1;
		}
		x = coord_native_to_amiga_x(x);
		if (y >= 0) {
			// On Android we already get correct line mapping from native2amiga_line_map.
			// Multiplying by 2 causes the pointer to hit max Y early (compressed range).
			#ifdef AMIBERRY_ANDROID
			y = coord_native_to_amiga_y(y);
			#else
...
static bool inputdevice_handle_inputcode2(int monid, int code, int state, const TCHAR *s)
{
	...
	case AKS_AUTO_CROP_IMAGE:
		...
		break;
	case AKS_OSK:
		// Toggle only on press. Some sources emit both press and release events;
		// without this guard VKBD will instantly toggle back off.
		if (state && vkbd_allowed(0))
			vkbd_toggle();
		break;
	...
	for (int j = 0; j <= IDTYPE_KEYBOARD; j++) {
		...
		for (int i = 0; i < num; i++) {
			if (df[i] == false) {
				...
				} else if (j == IDTYPE_JOYSTICK) {
					// Fresh joystick never seen before: auto-assign to port 1
					// (game joystick port) if it has no real joystick assigned,
					// or to port 0 as second choice.
					int target = -1;
					if (jportid[1] == JPORT_NONE || (jportid[1] >= JSEM_KBDLAYOUT && jportid[1] < JSEM_KBDLAYOUT + JSEM_LASTKBD)) {
						target = 1;
					} else if (jportid[0] == JPORT_NONE || (jportid[0] >= JSEM_KBDLAYOUT && jportid[0] < JSEM_KBDLAYOUT + JSEM_LASTKBD)) {
						target = 0;
					}
					if (target >= 0) {
						write_log(_T("Auto-assigning new joystick to port %d\n"), target);
						jportscustom[i] = -1;
						xfree(jports_name[target]);
						xfree(jports_configname[target]);
						jports_name[target] = my_strdup(idc.name);
						jports_configname[target] = my_strdup(idc.configname);
						jportsmode[target] = jportsmode[target]; // preserve existing mode
						jportid[target] = JSEM_JOYS; // mark as joystick
					} else {
						write_log(_T("Not inserted to any port (all occupied)\n"));
					}
				} else {
...
// Re-resolve joyport shortids that failed during initial config parsing
// because inputdevice_init() had not yet been called (i.e. idev[].get_num()==0).
// Must be called from main after inputdevice_init().
void inputdevice_fix_prefs_post_init(struct uae_prefs *p)
{
	bool changed = false;
	for (int i = 0; i < MAX_JPORTS; i++) {
		struct jport *jp = &jport_config_store[i];
		// Only re-resolve if port ended up NONE but had a joystick/mouse shortid
		if (p->jports[i].id == JPORT_NONE && jp->idc.shortid[0] &&
			(_tcsncmp(jp->idc.shortid, _T("joy"), 3) == 0 || _tcsncmp(jp->idc.shortid, _T("mouse"), 5) == 0)) {
			write_log(_T("Post-init: re-resolving port %d shortid '%s'\n"), i, jp->idc.shortid);
			if (inputdevice_joyport_config(p, jp->idc.shortid, NULL, i, jp->mode, jp->submode, 0, true)) {
				write_log(_T("Post-init: port %d resolved to id=%d\n"), i, p->jports[i].id);
				changed = true;
			} else {
				write_log(_T("Post-init: port %d shortid '%s' still unresolved\n"), i, jp->idc.shortid);
			}
		}
	}
	if (changed) {
		bool fixedports[MAX_JPORTS] = {};
		for (int i = 0; i < MAX_JPORTS; i++) {
			fixedports[i] = (p->jports[i].id != JPORT_NONE);
		}
		inputdevice_validate_jports(p, -1, fixedports);
		set_config_changed();
	}
}
