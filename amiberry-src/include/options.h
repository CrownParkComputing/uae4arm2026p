...
#else
// Android minimal builds do not include the desktop GUI stack (libguisan), but
// some option structs still reference gcn::Color. Provide a tiny compatible
// stand-in so the core can compile without pulling libguisan headers.
namespace gcn {
	struct Color {
		int r{};
		int g{};
...
struct amiberry_options
{
	...
	int virtual_joystick_alpha = 50;
	int virtual_joystick_scale = 100;
	bool virtual_joystick_haptic = false;
