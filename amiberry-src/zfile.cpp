...
static void manglefilename(const std::string& in, std::string& out, int outsize)
{
	...
	// Do NOT mangle content:// URIs: they intentionally start with "//" and any
	// replacement or deduplication would destroy the scheme (content:// → content:/).
	if (out.size() >= 10 && out.compare(0, 10, "content://") == 0) {
		return;
	}
#endif
