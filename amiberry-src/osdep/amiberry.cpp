...
#include "uae/uae.h"
#include "sana2.h"
#if !defined(AMIBERRY_ANDROID_MINIMAL)
...
static void setmouseactive2(AmigaMonitor* mon, int active, const bool allowpause)
{
	...
	// When the on-screen keyboard is shown, we must not enable mouse grab/relative mode.
	// On Android, relative mouse mode causes button events to report a constant position
	// (often the window center), which makes every tap hit the same VKBD key.
	if (active != 0 && vkbd_allowed(0) && vkbd_is_active()) {
		releasecapture(mon);
		recapture = 0;
		mouseactive = 0;
		return;
...
static void handle_window_event(const SDL_Event& event, AmigaMonitor* mon)
{
	...
	case SDL_WINDOWEVENT_SIZE_CHANGED:
		#ifdef AMIBERRY_ANDROID
		// Rotation/aspect changes can change renderer output size and viewport.
		// Refresh VKBD placement so hit-testing and cursor overlay stay aligned.
		if (vkbd_allowed(0))
...
static void map_window_to_renderer_coords(const AmigaMonitor* mon, int* x, int* y)
{
	if (!mon || !mon->amiga_window || !mon->amiga_renderer || !x || !y)
		return;

	// NOTE (Android): SDL_RenderWindowToLogical appears to produce incorrect
	// mappings under some device rotation/letterboxing setups (Y ends up too
	// small, so VKBD hit-testing always misses). Use a simple scale mapping
	// instead so coordinates match our VKBD drawing space.
	#ifndef AMIBERRY_ANDROID
	// Best-effort: let SDL perform the correct mapping when it supports it.
	// This accounts for renderer viewport, scaling, and logical size.
	#if SDL_VERSION_ATLEAST(2, 0, 18)
	{
		float lx = 0.0f;
		float ly = 0.0f;
		SDL_RenderWindowToLogical(mon->amiga_renderer, static_cast<float>(*x), static_cast<float>(*y), &lx, &ly);
	...
	int outW = 0;
	int outH = 0;
	SDL_GetRendererOutputSize(mon->amiga_renderer, &outW, &outH);
	if (outW <= 0 || outH <= 0)
		return;

	// Step 1: window -> renderer output coordinates.
	int ox = static_cast<int>((static_cast<long long>(*x) * outW) / ww);
	int oy = static_cast<int>((static_cast<long long>(*y) * outH) / wh);

	// Step 2: subtract renderer viewport (letterboxing/pillarboxing).
	SDL_Rect vp;
	SDL_RenderGetViewport(mon->amiga_renderer, &vp);
	if (vp.w <= 0 || vp.h <= 0)
	{
		vp.x = 0;
		vp.y = 0;
	...
	// Step 3: viewport -> renderer logical coordinates.
	int lw = 0;
	int lh = 0;
	...
	if (vp.w <= 0 || vp.h <= 0 || lw <= 0 || lh <= 0)
		return;

	*x = static_cast<int>((static_cast<long long>(ox) * lw) / vp.w);
	*y = static_cast<int>((static_cast<long long>(oy) * lh) / vp.h);

	// Clamp into logical bounds (touches landing in black bars should clamp).
	*x = std::clamp(*x, 0, lw - 1);
	*y = std::clamp(*y, 0, lh - 1);
}

static void map_output_to_logical_coords(const AmigaMonitor* mon, int* x, int* y)
{
	if (!mon || !mon->amiga_renderer || !x || !y)
		return;
	...
	if (vp.w > 0 && vp.h > 0 && lw > 0 && lh > 0)
	{
		*x = static_cast<int>((static_cast<long long>(xo) * lw) / vp.w);
		*y = static_cast<int>((static_cast<long long>(yo) * lh) / vp.h);
	}
}

#ifdef AMIBERRY_ANDROID
// On some Android devices/SDL builds, touchscreen input reaches us as mouse events.
// When VKBD is active we suppress mouse motion from affecting the emulator, but we
// still need to track the pointer position for accurate VKBD hit-testing.
static int vkbd_last_mouse_x = 0;
static int vkbd_last_mouse_y = 0;
static bool vkbd_last_mouse_valid = false;
static Uint32 vkbd_last_java_touch_ms = 0;

static bool vkbd_handle_finger_android(const AmigaMonitor* mon, const SDL_TouchFingerEvent& tfinger, bool down, int* outCode, int* outPressed)
{
	if (!mon || !mon->amiga_renderer)
		return false;
	...
	SDL_RenderGetLogicalSize(mon->amiga_renderer, &lw, &lh);
	if (lw == 0 || lh == 0)
		SDL_GetRendererOutputSize(mon->amiga_renderer, &lw, &lh);
	if (lw <= 0 || lh <= 0)
		return false;
	...
	const double nx = clamp01(static_cast<double>(tfinger.x));
	const double ny = clamp01(static_cast<double>(tfinger.y));

	// SDL touch coordinates on Android are device-normalized and can be rotated
	// relative to our renderer logical coordinates depending on device orientation.
	...
	// For releases we don't need coordinates; vkbd_handle_pointer() will send key-up
	// for the last pressed key. Always do this to avoid stuck keys.
	if (!down)
	{
		int code = 0;
		int pressed = 0;
		const bool handled = vkbd_handle_pointer(0, 0, false, &code, &pressed);
		if (handled)
		{
	...
	{
		int x = static_cast<int>(c.nx * lw);
		int y = static_cast<int>(c.ny * lh);
		if (x < 0) x = 0;
		if (y < 0) y = 0;
		if (x >= lw) x = lw - 1;
		if (y >= lh) y = lh - 1;
		...
		int code = 0;
		int pressed = 0;
		const bool handled = vkbd_handle_pointer(x, y, true, &code, &pressed);
		if (handled)
		{
			if (outCode) *outCode = code;
			if (outPressed) *outPressed = pressed;
			#if defined(AMIBERRY_VKBD_DEBUG_LOG)
			__android_log_print(ANDROID_LOG_INFO, "VKBD",
				"finger map=%s norm=%.4f,%.4f -> xy=%d,%d lw=%d lh=%d handled=1 code=%d pressed=%d",
				c.name, c.nx, c.ny, x, y, lw, lh, code, pressed);
			#endif
			return true;
		}
	}
...
static bool vkbd_handle_mouse_android(const AmigaMonitor* mon, int rawX, int rawY, bool down, int* outCode, int* outPressed)
{
	if (!mon || !mon->amiga_window || !mon->amiga_renderer)
		return false;
	...
	double nx = static_cast<double>(rawX) / static_cast<double>(ww);
	double ny = static_cast<double>(rawY) / static_cast<double>(wh);
	if (nx < 0.0) nx = 0.0;
	if (ny < 0.0) ny = 0.0;
	if (nx > 1.0) nx = 1.0;
	if (ny > 1.0) ny = 1.0;

	// For releases we don't need coordinates; always send key-up for the last pressed key
	// to avoid stuck keys when touch->mouse emulation drops/warps the UP position.
	if (!down)
	{
		int code = 0;
		int pressed = 0;
	...
	struct Candidate { const char* name; double nx; double ny; };
	const Candidate candidates[] = {
		{"n", nx, ny},
		{"invY", nx, 1.0 - ny},
		{"rot90", ny, 1.0 - nx},
	...
	SDL_Rect kb;
	const bool haveKb = vkbd_get_bounds(&kb);

	// If we can compute the VKBD rectangle in *window* coordinates, pick the transform
	// that preserves the tap's relative position within the keyboard.
	...
	if (haveKb)
	{
		...
		if (outW > 0 && outH > 0 && vp.w > 0 && vp.h > 0 && kb.w > 0 && kb.h > 0)
		{
			// Project kb (logical) -> output -> window
			const double kbOutX = vp.x + (static_cast<double>(kb.x) * vp.w) / lw;
			const double kbOutY = vp.y + (static_cast<double>(kb.y) * vp.h) / lh;
			const double kbOutW = (static_cast<double>(kb.w) * vp.w) / lw;
			const double kbOutH = (static_cast<double>(kb.h) * vp.h) / lh;
			const double kbWinX = (kbOutX * ww) / outW;
			const double kbWinY = (kbOutY * wh) / outH;
			const double kbWinW = (kbOutW * ww) / outW;
			const double kbWinH = (kbOutH * wh) / outH;

			if (kbWinW > 1.0 && kbWinH > 1.0)
			{
				// Relative position of the tap inside the keyboard in window coords
				const double rx = (static_cast<double>(rawX) - kbWinX) / kbWinW;
				const double ry = (static_cast<double>(rawY) - kbWinY) / kbWinH;

				for (const auto& c : candidates)
				{
					const double cx = c.nx;
					const double cy = c.ny;
					const int x = std::clamp(static_cast<int>(cx * lw), 0, lw - 1);
					const int y = std::clamp(static_cast<int>(cy * lh), 0, lh - 1);

					// Must land inside the VKBD bounds to be a candidate.
					if (x < kb.x || y < kb.y || x >= (kb.x + kb.w) || y >= (kb.y + kb.h))
						continue;

					// Compare relative position inside VKBD.
					const double mx = (static_cast<double>(x) - kb.x) / kb.w;
					const double my = (static_cast<double>(y) - kb.y) / kb.h;
					const double dx = mx - rx;
					const double dy = my - ry;
	...
	// Fallback: first transform that hits a key.
	for (const auto& c : candidates)
	{
		if (best)
			break;
		...
		int code = 0;
		int pressed = 0;
		if (vkbd_handle_pointer(x, y, true, &code, &pressed))
		{
			best = &c;
			break;
		}
	...
	if (!best)
	{
		#if defined(AMIBERRY_VKBD_DEBUG_LOG)
		__android_log_print(ANDROID_LOG_INFO, "VKBD",
			"mouse map=MISS raw=%d,%d norm=%.4f,%.4f ww=%d wh=%d lw=%d lh=%d",
			rawX, rawY, nx, ny, ww, wh, lw, lh);
		#endif
		return false;
	}

	const int x = std::clamp(static_cast<int>(best->nx * lw), 0, lw - 1);
	const int y = std::clamp(static_cast<int>(best->ny * lh), 0, lh - 1);
	int code = 0;
	int pressed = 0;
	const bool handled = vkbd_handle_pointer(x, y, true, &code, &pressed);
	if (handled)
	{
		...
		#if defined(AMIBERRY_VKBD_DEBUG_LOG)
		__android_log_print(ANDROID_LOG_INFO, "VKBD",
			"mouse map=%s raw=%d,%d norm=%.4f,%.4f -> xy=%d,%d lw=%d lh=%d handled=1 code=%d pressed=%d score=%.6f",
			best->name, rawX, rawY, best->nx, best->ny, x, y, lw, lh, code, pressed, bestScore);
		#endif
	}
	return handled;
}
...
static void handle_finger_event(const SDL_Event& event, const AmigaMonitor* mon)
{
	if (!isfocus())
		return;

	#ifdef AMIBERRY_ANDROID
	if ((event.tfinger.type == SDL_FINGERDOWN || event.tfinger.type == SDL_FINGERUP) && mon && mon->amiga_window)
	{
		int ww = 0;
		int wh = 0;
		int outW = 0;
		...
		__android_log_print(
			ANDROID_LOG_INFO,
			"VKBD",
			"raw touch %s finger=%lld norm=%.4f,%.4f (ww=%d wh=%d outW=%d outH=%d) vkbd_allowed=%d vkbd_active=%d",
			event.tfinger.type == SDL_FINGERDOWN ? "DOWN" : "UP",
	...
	#ifdef AMIBERRY_ANDROID
	// If SDL is already emulating touch as mouse input, avoid also handling it via
	// SDL_FINGER* paths (except for VKBD). This prevents double movement/clicks.
	auto touch_mouse_enabled = []() -> bool {
		#ifdef SDL_HINT_TOUCH_MOUSE_EVENTS
		const char* v = SDL_GetHint(SDL_HINT_TOUCH_MOUSE_EVENTS);
		#else
		const char* v = SDL_GetHint("SDL_TOUCH_MOUSE_EVENTS");
		#endif
		return v && strcmp(v, "0") != 0;
	};
	if (touch_mouse_enabled())
		return;
	#endif
	#endif

	// When VKBD is active, treat taps as VKBD key presses/releases.
	if (vkbd_allowed(0) && vkbd_is_active() && mon && mon->amiga_window)
	{
		#ifdef AMIBERRY_ANDROID
		releasecapture(mon);
		#endif
		const bool down = (event.tfinger.type == SDL_FINGERDOWN);
		int code = 0;
		int pressed = 0;
		bool handled = false;
		#ifdef AMIBERRY_ANDROID
		handled = vkbd_handle_finger_android(mon, event.tfinger, down, &code, &pressed);
		#else
		int w = 0;
		int h = 0;
		...
		if (w > 0 && h > 0)
		{
			int x = static_cast<int>(event.tfinger.x * w);
			int y = static_cast<int>(event.tfinger.y * h);
			map_window_to_renderer_coords(mon, &x, &y);
			handled = vkbd_handle_pointer(x, y, down, &code, &pressed);
		}
		#endif
			#ifdef AMIBERRY_ANDROID
			#if defined(AMIBERRY_VKBD_DEBUG_LOG)
			__android_log_print(ANDROID_LOG_INFO, "VKBD", "touch %s finger=%lld handled=%d code=%d pressed=%d norm=%.4f,%.4f",
				down ? "DOWN" : "UP", (long long)event.tfinger.fingerId, handled ? 1 : 0, code, pressed, (double)event.tfinger.x, (double)event.tfinger.y);
			write_log(_T("VKBD touch %s finger=%lld handled=%d code=%d pressed=%d norm=%.4f,%.4f\n"),
				down ? _T("DOWN") : _T("UP"), (long long)event.tfinger.fingerId, handled ? 1 : 0, code, pressed, (double)event.tfinger.x, (double)event.tfinger.y);
			#endif
			#endif
		if (handled)
		{
	...
	// Legacy behavior: only handle the primary finger.
...
static void handle_mouse_button_event(const SDL_Event& event, const AmigaMonitor* mon)
{
	...
	// If VKBD is active, let it consume left clicks for key press/release.
	if (vkbd_allowed(0) && vkbd_is_active() && isfocus() && button == SDL_BUTTON_LEFT)
	{
		#ifdef AMIBERRY_ANDROID
		// Prefer Java-forwarded MotionEvent touches when they are flowing, but keep a fallback
		// path so VKBD remains usable if that bridge breaks on some devices/SDL builds.
		const Uint32 now = SDL_GetTicks();
		const bool javaActive = (vkbd_last_java_touch_ms != 0 && (now - vkbd_last_java_touch_ms) < 750);
		if (javaActive)
		{
			#if defined(AMIBERRY_VKBD_DEBUG_LOG)
			__android_log_print(ANDROID_LOG_INFO, "VKBD", "mouse button ignored (java touch active) raw=%d,%d rel=%d which=%u",
				event.button.x, event.button.y, SDL_GetRelativeMouseMode() ? 1 : 0, (unsigned)event.button.which);
			#endif
			return;
		}

		// Touch->mouse emulation on Android can yield a constant button position,
		// which makes every tap hit the same VKBD key (often perceived as 'always W').
		// Prefer SDL_FINGER events for touch. Still accept real mouse clicks.
		// However, if Java VKBD touch forwarding isn't working on this device, allow
		// touch-mouse clicks as a fallback *when* we have a recent motion position.
		if (event.button.which == SDL_TOUCH_MOUSEID && !vkbd_last_mouse_valid)
		{
			#if defined(AMIBERRY_VKBD_DEBUG_LOG)
			__android_log_print(ANDROID_LOG_INFO, "VKBD", "mouse event from touch ignored (SDL_TOUCH_MOUSEID, no motion) raw=%d,%d rel=%d",
				event.button.x, event.button.y, SDL_GetRelativeMouseMode() ? 1 : 0);
			#endif
			return;
		}

		// VKBD hit-testing needs absolute mouse/touch coordinates.
		// If SDL is in relative mouse mode (pointer capture), button events will
		// report a constant position (often the window center), causing misses.
		releasecapture(mon);
		#endif
		...
		bool handled = false;
		#ifdef AMIBERRY_ANDROID
		int rx = event.button.x;
		int ry = event.button.y;
		// Some Android/SDL input paths deliver touchscreen as mouse events where
		// button x/y can be constant/stale. Use motion-updated position if available,
		// if available.
		if (vkbd_last_mouse_valid)
		{
			rx = vkbd_last_mouse_x;
			ry = vkbd_last_mouse_y;
		}
		handled = vkbd_handle_mouse_android(mon, rx, ry, state, &code, &pressed);
		#else
		int ww = 0;
		int wh = 0;
		int outW = 0;
		...
		if (ww > 0 && wh > 0 && outW > 0 && outH > 0)
		{
			x = static_cast<int>((static_cast<long long>(x) * outW) / ww);
			y = static_cast<int>((static_cast<long long>(y) * outH) / wh);
		}
		map_output_to_logical_coords(mon, &x, &y);
		handled = vkbd_handle_pointer(x, y, state, &code, &pressed);
		#endif
		#ifdef AMIBERRY_ANDROID
		#if defined(AMIBERRY_VKBD_DEBUG_LOG)
		__android_log_print(ANDROID_LOG_INFO, "VKBD", "mouse %s raw=%d,%d handled=%d code=%d pressed=%d",
			state ? "DOWN" : "UP", event.button.x, event.button.y, handled ? 1 : 0, code, pressed);
		write_log(_T("VKBD mouse %s raw=%d,%d handled=%d code=%d pressed=%d\n"),
			state ? _T("DOWN") : _T("UP"), event.button.x, event.button.y, handled ? 1 : 0, code, pressed);
		#endif
		#endif
		if (handled)
		{
...
static void handle_finger_motion_event(const SDL_Event& event)
{
	...
	#ifdef AMIBERRY_ANDROID
	// If SDL is emulating touch as mouse motion, prefer that path for pointer control.
	#ifdef SDL_HINT_TOUCH_MOUSE_EVENTS
	const char* v = SDL_GetHint(SDL_HINT_TOUCH_MOUSE_EVENTS);
	#else
	const char* v = SDL_GetHint("SDL_TOUCH_MOUSE_EVENTS");
	...
	// SDL finger coordinates are normalized floats (0..1). setmousestate() expects
	// absolute pixel coordinates when isabs=1.
	...
	const double nx = clamp01(static_cast<double>(event.tfinger.x));
	const double ny = clamp01(static_cast<double>(event.tfinger.y));
	// Keep finger absolute coordinates in window/native space. Downstream mousehack
	// conversion uses getgfxoffset() to scale into emulated output space.
	int x = std::clamp(static_cast<int>(nx * (double)ww), 0, ww - 1);
	int y = std::clamp(static_cast<int>(ny * (double)wh), 0, wh - 1);
	// Convert window coords into the same logical space VKBD/renderer uses.
	map_window_to_renderer_coords(mon, &x, &y);
...
static void handle_mouse_motion_event(const SDL_Event& event, const AmigaMonitor* mon)
{
	if (vkbd_allowed(0) && vkbd_is_active())
	{
		...
		map_window_to_renderer_coords(mon, &mx, &my);
		vkbd_last_mouse_x = mx;
		vkbd_last_mouse_y = my;
		vkbd_last_mouse_valid = true;
		#endif
		return;
	}
	...
	{
		/* absolute */
		// Map window coordinates into renderer logical coordinates so absolute input
		// matches the same space that rendering/hit-testing uses.
		map_window_to_renderer_coords(mon, &x, &y);
		setmousestate(0, 0, x, 1);
		setmousestate(0, 1, y, 1);
	}
...
static void process_event(const SDL_Event& event)
{
	...
	#ifdef AMIBERRY_ANDROID
	// VKBD touch events forwarded from Java (real MotionEvent coordinates).
	extern Uint32 g_amiberry_vkbd_touch_event_type;
	extern Uint32 g_amiberry_android_media_event_type;
	...
	if (g_amiberry_vkbd_touch_event_type != 0 && event.type == g_amiberry_vkbd_touch_event_type)
	{
		auto* payload = static_cast<AmiberryVkbdTouchPayload*>(event.user.data1);
		if (!payload)
			return;

		const int action = payload->action;
		const int rawX = payload->x;
		const int rawY = payload->y;
		const int vw = payload->viewW;
		const int vh = payload->viewH;
		SDL_free(payload);

		const bool down = (action == 0 /*ACTION_DOWN*/ || action == 2 /*ACTION_MOVE*/);
		const bool up = (action == 1 /*ACTION_UP*/ || action == 3 /*ACTION_CANCEL*/);
		if (!down && !up)
			return;

		if (!(vkbd_allowed(0) && vkbd_is_active()) || !mon || !mon->amiga_renderer || vw <= 0 || vh <= 0)
			return;

		if (!mon->amiga_window)
			return;

		// Ensure we remain in absolute mode while VKBD is visible.
		releasecapture(mon);
		vkbd_last_java_touch_ms = SDL_GetTicks();
		...
		// Normalize from Java view coords and try the common Android orientation transforms.
		auto clamp01 = [](double v) {
			if (v < 0.0) return 0.0;
			if (v > 1.0) return 1.0;
			return v;
		};
		const double nx = clamp01((double)rawX / (double)vw);
		const double ny = clamp01((double)rawY / (double)vh);
		...
		if (!down)
		{
			handled = vkbd_handle_pointer(0, 0, false, &code, &pressed);
		}
		else
		{
			for (const auto& c : candidates)
			{
				// Map from Java view space -> SDL window space. This is important on devices
				// where the Android view is scaled but the SDL window/surface buffer is not.
				int x = std::clamp((int)(c.nx * (double)ww), 0, ww - 1);
				int y = std::clamp((int)(c.ny * (double)wh), 0, wh - 1);

				// Convert window coords -> renderer output coords -> renderer logical coords.
				// This accounts for renderer viewport/letterboxing, which is common on Android
				// and would otherwise skew VKBD hit-testing (e.g. Shift hitting X).
				int outW = 0;
				int outH = 0;
				SDL_GetRendererOutputSize(mon->amiga_renderer, &outW, &outH);
				if (outW > 0 && outH > 0 && ww > 0 && wh > 0)
				{
					x = std::clamp((int)((static_cast<long long>(x) * outW) / ww), 0, outW - 1);
					y = std::clamp((int)((static_cast<long long>(y) * outH) / wh), 0, outH - 1);
					map_output_to_logical_coords(mon, &x, &y);
				}
				else
				{
					// Fallback: best-effort window->logical mapping.
					map_window_to_renderer_coords(mon, &x, &y);
				}
				...
				if (vkbd_handle_pointer(x, y, true, &code, &pressed))
				{
					#if defined(AMIBERRY_VKBD_DEBUG_LOG)
					__android_log_print(ANDROID_LOG_INFO, "VKBD",
						"java touch map=%s raw=%d,%d view=%dx%d -> xy=%d,%d win=%dx%d code=%d pressed=%d",
						c.name, rawX, rawY, vw, vh, x, y, ww, wh, code, pressed);
					#endif
					handled = true;
					break;
				}
	...
	extern Uint32 g_amiberry_android_savestate_event_type;
	if (g_amiberry_android_savestate_event_type != 0 && event.type == g_amiberry_android_savestate_event_type)
	{
		auto* payload = static_cast<AmiberrySaveStatePayload*>(event.user.data1);
		if (!payload)
			return;

		int slot = payload->slot;
		int mode = payload->mode; // 1=save, 0=load
		SDL_free(payload);

		// Use Amiberry's quick save/load logic which handles slots (1-based).
		// Java sends 0-9, so we add 1.
		if (savestate_fname[0] == 0)
			target_setdefaultstatefilename(nullptr);
		savestate_quick(slot + 1, mode);
		return;
	}
	if (g_amiberry_android_media_event_type != 0 && event.type == g_amiberry_android_media_event_type)
	{
		auto* payload = static_cast<AmiberryMediaSwapPayload*>(event.user.data1);
		if (!payload)
			return;

		const int drive = payload->drive;
		...
		// Best-effort: insert immediately. This mirrors drag-and-drop behavior.
		if (drive >= 0 && drive <= 3)
		{
			if (path[0] == 0)
			{
	...
	extern Uint32 g_amiberry_virtual_joy_event_type;
	extern std::atomic<int> g_amiberry_virtual_joy_enabled;
	if (g_amiberry_virtual_joy_event_type != 0 && event.type == g_amiberry_virtual_joy_event_type)
	{
		auto* payload = static_cast<AmiberryVirtualJoyPayload*>(event.user.data1);
		if (!payload)
			return;

		int axis = payload->axis;
		int val = payload->value;
		int btn = payload->button;
		int pressed = payload->pressed;
		SDL_free(payload);

		// Check if virtual joystick is enabled - if not, skip processing
		if (!g_amiberry_virtual_joy_enabled.load(std::memory_order_relaxed))
			return;

		const auto is_port_joylike = [](const int mode) {
			return mode == JSEM_MODE_JOYSTICK
				|| mode == JSEM_MODE_GAMEPAD
				|| mode == JSEM_MODE_JOYSTICK_ANALOG
				|| mode == JSEM_MODE_JOYSTICK_CD32;
		};

		const bool port0_joylike = is_port_joylike(currprefs.jports[0].mode);
		const bool port1_joylike = is_port_joylike(currprefs.jports[1].mode);

		int target_port = 1;
		if (port1_joylike)
			target_port = 1;
		else if (port0_joylike)
			target_port = 0;

		const bool target_cd32 = currprefs.jports[target_port].mode == JSEM_MODE_JOYSTICK_CD32;

		if (axis >= 0) {
			if (axis == 0)
				inputdevice_uaelib(target_port == 0 ? _T("JOY1_HORIZ") : _T("JOY2_HORIZ"), val, 1, false);
			else if (axis == 1)
				inputdevice_uaelib(target_port == 0 ? _T("JOY1_VERT") : _T("JOY2_VERT"), val, 1, false);
		}
		if (btn >= 0) {
			if (target_cd32) {
				if (btn == 0)
					inputdevice_uaelib(target_port == 0 ? _T("JOY1_CD32_RED") : _T("JOY2_CD32_RED"), pressed, 1, false);
				else if (btn == 1)
					inputdevice_uaelib(target_port == 0 ? _T("JOY1_CD32_BLUE") : _T("JOY2_CD32_BLUE"), pressed, 1, false);
			} else {
				if (btn == 0)
					inputdevice_uaelib(target_port == 0 ? _T("JOY1_FIRE_BUTTON") : _T("JOY2_FIRE_BUTTON"), pressed, 1, false);
				else if (btn == 1)
					inputdevice_uaelib(target_port == 0 ? _T("JOY1_2ND_BUTTON") : _T("JOY2_2ND_BUTTON"), pressed, 1, false);
			}
		}
		return;
	}
...
{
	// Android SAF support:
	// When the launcher selects media via SAF (content://), the Java layer opens a
	// ParcelFileDescriptor and passes /proc/self/fd/<n> into the core.
	//
	// Calling realpath() on /proc/self/fd/<n> resolves to the underlying storage
	// location (e.g. /storage/emulated/0/...), which can then fail under scoped
	// storage (permission denied) because the app does not have direct filesystem
	// access. Keep /proc/*/fd/* paths as-is.
	if (path && path[0] != 0) {
		if (!_tcsnicmp(path, _T("content://"), 10)) {
			return;
		}
		if (!_tcsnicmp(path, _T("/proc/self/fd/"), 14) || _tcsstr(path, _T("/fd/")) != nullptr) {
			return;
		}
	}
...
void save_amiberry_settings()
{
	...
	// Virtual Joypad
	write_int_option("virtual_joystick_alpha", amiberry_options.virtual_joystick_alpha);
	write_int_option("virtual_joystick_scale", amiberry_options.virtual_joystick_scale);
	write_bool_option("virtual_joystick_haptic", amiberry_options.virtual_joystick_haptic);
...
static int parse_amiberry_settings_line(const char *path, char *linea)
{
	...
	else
	{
		...
		const std::string prev_logfile_path = logfile_path;
		const bool prev_write_logfile = amiberry_options.write_logfile;
		const int logfile_path_matched = cfgfile_string(option, value, "logfile_path", logfile_path);
		ret |= logfile_path_matched;
		...
		const int write_logfile_matched = cfgfile_yesno(option, value, "write_logfile", &amiberry_options.write_logfile);
		ret |= write_logfile_matched;
		if ((write_logfile_matched && !prev_write_logfile && amiberry_options.write_logfile)
			|| (logfile_path_matched && (logfile_path != prev_logfile_path) && amiberry_options.write_logfile))
		{
			// logging_init() is called before normal config/-s parsing.
			// Re-initialize logging once these options are actually set.
			logging_init();
		}
		...
		ret |= cfgfile_intval(option, value, "virtual_joystick_alpha", &amiberry_options.virtual_joystick_alpha, 1);
		ret |= cfgfile_intval(option, value, "virtual_joystick_scale", &amiberry_options.virtual_joystick_scale, 1);
		ret |= cfgfile_yesno(option, value, "virtual_joystick_haptic", &amiberry_options.virtual_joystick_haptic);
...
std::string get_data_directory(bool portable_mode)
{
	...
	if (internal && internal[0]) {
		return std::string(internal) + "/amiberry/data/";
	}
	// Fallback: if SDL didn't provide a path, use current working directory.
	char cwd_tmp[MAX_DPATH];
	getcwd(cwd_tmp, MAX_DPATH);
	return std::string(cwd_tmp) + "/data/";
	#endif
...
std::string get_home_directory(const bool portable_mode)
{
	...
	const char* internal = SDL_AndroidGetInternalStoragePath();
	if (internal && internal[0]) {
		write_log("Android: Using internal storage as home directory: %s\n", internal);
		return std::string(internal) + "/amiberry";
	}
	write_log("Android: SDL_AndroidGetInternalStoragePath returned null; falling back to cwd\n");
	char cwd_tmp[MAX_DPATH];
	getcwd(cwd_tmp, MAX_DPATH);
	return {cwd_tmp};
	#endif
...
std::string get_config_directory(bool portable_mode)
{
	...
	const char* internal = SDL_AndroidGetInternalStoragePath();
	if (internal && internal[0]) {
		return std::string(internal) + "/amiberry/conf";
	}
	char cwd_tmp[MAX_DPATH];
	getcwd(cwd_tmp, MAX_DPATH);
	return std::string(cwd_tmp) + "/conf";
	#endif
...
std::string get_plugins_directory(bool portable_mode)
{
	...
	const char* internal = SDL_AndroidGetInternalStoragePath();
	if (internal && internal[0]) {
		return std::string(internal) + "/amiberry/plugins";
	}
	char cwd_tmp[MAX_DPATH];
	getcwd(cwd_tmp, MAX_DPATH);
	return std::string(cwd_tmp) + "/plugins";
	#endif
...
int amiberry_run(int argc, char* argv[])
{
	...
	#ifdef AMIBERRY_ANDROID
	// On Android, prefer SDL's touch->mouse emulation for general pointer control,
	// because it tends to align better with the actual surface/window mapping.
	// We keep mouse->touch disabled to avoid synthetic touch coordinates.
	#ifdef SDL_HINT_TOUCH_MOUSE_EVENTS
