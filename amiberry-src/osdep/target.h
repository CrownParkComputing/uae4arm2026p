/*tor should reset if new disk inseerted in df0
...
#if defined(__ANDROID__)
	// Support Storage Access Framework URIs (content://...) by opening a detached FD
	// on the Java side and wrapping it as a FILE*.
	// This is required for hardfiles when the launcher stores content URIs.
	if (strncmp(path, "content://", 10) == 0) {
		JNIEnv* env = static_cast<JNIEnv*>(SDL_AndroidGetJNIEnv());
		if (env) {
			// Try direct FindClass first; fall back to Activity classloader for native threads.
			jclass bridgeCls = env->FindClass("com/uae4arm2026/SafFileBridge");
			if (!bridgeCls) {
				env->ExceptionClear();
				// Classloader fallback (required when called from a non-Java-attached thread).
				jobject activity = reinterpret_cast<jobject>(SDL_AndroidGetActivity());
				if (activity) {
					jclass actCls = env->GetObjectClass(activity);
					if (actCls) {
						jmethodID midGCL = env->GetMethodID(actCls, "getClassLoader", "()Ljava/lang/ClassLoader;");
						if (midGCL) {
							jobject cl = env->CallObjectMethod(activity, midGCL);
							if (cl) {
								jclass clCls = env->FindClass("java/lang/ClassLoader");
								if (clCls) {
									jmethodID midLC = env->GetMethodID(clCls, "loadClass", "(Ljava/lang/String;)Ljava/lang/Class;");
									if (midLC) {
										jstring jname = env->NewStringUTF("com.uae4arm2026.SafFileBridge");
										if (jname) {
											jobject clsObj = env->CallObjectMethod(cl, midLC, jname);
											env->DeleteLocalRef(jname);
											if (clsObj && !env->ExceptionCheck()) {
												bridgeCls = static_cast<jclass>(clsObj);
											} else {
												env->ExceptionClear();
											}
										}
			...
			if (!bridgeCls) {
				...
				if (!mid) {
					...
					// Map fopen mode to PFD mode.
					const char* pfdMode = "r";
					if (mode[0] == 'w') {
						pfdMode = "rwt";
					} else if (mode[0] == 'a') {
						pfdMode = "wa";
					} else if (mode[0] == 'r') {
						pfdMode = (strchr(mode, '+') != nullptr) ? "rw" : "r";
					}

					jstring juri = env->NewStringUTF(path);
					jstring jmode = env->NewStringUTF(pfdMode);
					if (juri && jmode) {
						...
						if (fd >= 0) {
							FILE* f = fdopen(static_cast<int>(fd), mode);
							if (f) {
								return f;
							}
