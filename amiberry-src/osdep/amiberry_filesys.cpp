...
#if defined(__ANDROID__)
	bool is_saf{};
	std::vector<std::string> saf_entries{};
	size_t saf_index{};
#endif
};
...
#if defined(__ANDROID__)
static bool is_content_uri_path(const char* s)
{
	return s != nullptr && (strncmp(s, "content://", 10) == 0);
}

static bool is_tree_uri_path(const char* s)
{
	return is_content_uri_path(s) && strstr(s, "::") != nullptr;
}

static void split_tree_path(const char* path, std::string& outTreeUri, std::string& outRelPath)
{
	outTreeUri.clear();
	outRelPath.clear();
	if (!path) return;
	...
	outTreeUri.assign(path, sep - path);
	outRelPath.assign(sep + 2);
}

static const char* flags_to_pfd_mode(int flags)
{
	const bool rdwr = (flags & O_RDWR) == O_RDWR;
	const bool wronly = (flags & O_WRONLY) == O_WRONLY;
	const bool trunc = (flags & O_TRUNC) == O_TRUNC;
	const bool append = (flags & O_APPEND) == O_APPEND;

	if (rdwr) {
		// Read/write, truncate if requested.
		return trunc ? "rwt" : "rw";
	}
	if (wronly) {
		// Write-only. Prefer append if requested.
		if (append) return "wa";
		return "w";
	}
	return "r";
...
static jclass load_class_with_activity_classloader(JNIEnv* env, const char* fqcn)
{
	if (!env || !fqcn) return nullptr;

	// First try FindClass (works if we are on a Java-attached thread with correct classloader).
	{
	...
	// Fallback: use Activity's ClassLoader.
	jobject activity = reinterpret_cast<jobject>(SDL_AndroidGetActivity());
	if (!activity) return nullptr;

	jclass activityCls = env->GetObjectClass(activity);
	if (!activityCls) return nullptr;

	jmethodID midGetClassLoader = env->GetMethodID(activityCls, "getClassLoader", "()Ljava/lang/ClassLoader;");
	if (!midGetClassLoader) {
		env->ExceptionClear();
		return nullptr;
	}

	jobject classLoader = env->CallObjectMethod(activity, midGetClassLoader);
	if (!classLoader) {
		env->ExceptionClear();
		return nullptr;
	}
	...
	jmethodID midLoadClass = env->GetMethodID(clCls, "loadClass", "(Ljava/lang/String;)Ljava/lang/Class;");
	if (!midLoadClass) {
		env->ExceptionClear();
		return nullptr;
	}

	// Convert path-style class name to dotted name for ClassLoader.loadClass.
	std::string dotted(fqcn);
	std::replace(dotted.begin(), dotted.end(), '/', '.');
	jstring jname = env->NewStringUTF(dotted.c_str());
	if (!jname) return nullptr;

	jobject clsObj = env->CallObjectMethod(classLoader, midLoadClass, jname);
	...
	return static_cast<jclass>(clsObj);
}

static int android_open_detached_fd_for_uri(const char* uri, int flags)
{
	JNIEnv* env = static_cast<JNIEnv*>(SDL_AndroidGetJNIEnv());
	if (!env || !uri) return -1;

	jclass bridgeCls = load_class_with_activity_classloader(env, "com/uae4arm2026/SafFileBridge");
	if (!bridgeCls) return -1;

	if (is_tree_uri_path(uri)) {
		std::string treeUri;
		std::string relPath;
		split_tree_path(uri, treeUri, relPath);

		jmethodID mid = env->GetStaticMethodID(bridgeCls, "openDetachedFdTreePath", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)I");
		if (!mid) {
			env->ExceptionClear();
			return -1;
		}

		jstring jtree = env->NewStringUTF(treeUri.c_str());
		jstring jrel = env->NewStringUTF(relPath.c_str());
		jstring jmode = env->NewStringUTF(flags_to_pfd_mode(flags));
		if (!jtree || !jrel || !jmode) {
			if (jtree) env->DeleteLocalRef(jtree);
			if (jrel) env->DeleteLocalRef(jrel);
			if (jmode) env->DeleteLocalRef(jmode);
			return -1;
		}

		const jint fd = env->CallStaticIntMethod(bridgeCls, mid, jtree, jrel, jmode);
	...
	jmethodID mid = env->GetStaticMethodID(bridgeCls, "openDetachedFd", "(Ljava/lang/String;Ljava/lang/String;)I");
	if (!mid) {
		env->ExceptionClear();
		return -1;
	}
	...
	const jint fd = env->CallStaticIntMethod(bridgeCls, mid, juri, jmode);
	if (env->ExceptionCheck()) {
		env->ExceptionClear();
	}
...
static bool android_uri_exists(const char* uri)
{
	JNIEnv* env = static_cast<JNIEnv*>(SDL_AndroidGetJNIEnv());
	...
	if (is_tree_uri_path(uri)) {
		...
		std::string relPath;
		split_tree_path(uri, treeUri, relPath);
		jmethodID mid = env->GetStaticMethodID(bridgeCls, "existsTreePath", "(Ljava/lang/String;Ljava/lang/String;)Z");
		if (!mid) {
			env->ExceptionClear();
			return false;
		}
		...
		const jboolean ok = env->CallStaticBooleanMethod(bridgeCls, mid, jtree, jrel);
		if (env->ExceptionCheck()) {
			env->ExceptionClear();
		}
		env->DeleteLocalRef(jtree);
	...
	jmethodID mid = env->GetStaticMethodID(bridgeCls, "exists", "(Ljava/lang/String;)Z");
	if (!mid) {
		env->ExceptionClear();
		return false;
	}
	...
	const jboolean ok = env->CallStaticBooleanMethod(bridgeCls, mid, juri);
	if (env->ExceptionCheck()) {
		env->ExceptionClear();
	}
	env->DeleteLocalRef(juri);
...
static bool android_uri_is_directory(const char* uri)
{
	JNIEnv* env = static_cast<JNIEnv*>(SDL_AndroidGetJNIEnv());
	if (!env || !uri) return false;
	...
	if (is_tree_uri_path(uri)) {
		...
		std::string relPath;
		split_tree_path(uri, treeUri, relPath);
		jmethodID mid = env->GetStaticMethodID(bridgeCls, "isDirectoryTreePath", "(Ljava/lang/String;Ljava/lang/String;)Z");
		if (!mid) {
			env->ExceptionClear();
			return false;
		}
	...
	jmethodID mid = env->GetStaticMethodID(bridgeCls, "isDirectory", "(Ljava/lang/String;)Z");
	if (!mid) {
		env->ExceptionClear();
		return false;
	}
...
static bool android_list_children_tree_path(const char* treePath, std::vector<std::string>& outNames)
{
	outNames.clear();
	JNIEnv* env = static_cast<JNIEnv*>(SDL_AndroidGetJNIEnv());
	...
	std::string treeUri;
	std::string relPath;
	split_tree_path(treePath, treeUri, relPath);

	jmethodID mid = env->GetStaticMethodID(bridgeCls, "listChildrenTreePath", "(Ljava/lang/String;Ljava/lang/String;)[Ljava/lang/String;");
	if (!mid) {
		env->ExceptionClear();
		return false;
	}
	...
	auto arr = (jobjectArray)env->CallStaticObjectMethod(bridgeCls, mid, jtree, jrel);
	if (env->ExceptionCheck()) {
		env->ExceptionClear();
	}
	env->DeleteLocalRef(jtree);
...
{
#if defined(__ANDROID__)
	// Android NDK does not provide iconv; for now, keep filenames as UTF-8.
	output.assign(input);
	return true;
#else
...
#if defined(__ANDROID__)
	if (is_tree_uri_path(output.c_str()) && android_uri_is_directory(output.c_str())) {
		mod->is_saf = true;
		mod->saf_index = 0;
		android_list_children_tree_path(output.c_str(), mod->saf_entries);
		mod->dir = nullptr;
		return mod.release();
	}
	if (is_content_uri_path(output.c_str())) {
		write_log("my_opendir: content URI is not a directory path: %s\n", output.c_str());
		return nullptr;
	}
#endif
...
#if defined(__ANDROID__)
		if (mod->is_saf) {
			while (mod->saf_index < mod->saf_entries.size() && result.empty()) {
				result = mod->saf_entries[mod->saf_index++];
			}
...
bool my_existsfile(const char* name)
{
	if (!name) return false;
#if defined(__ANDROID__)
	if (is_content_uri_path(name)) {
		// DocumentFile reports existence; treat non-directories as files.
		return android_uri_exists(name) && !android_uri_is_directory(name);
	}
#endif
...
#if defined(__ANDROID__)
	if (is_content_uri_path(name)) {
		return android_uri_is_directory(name);
	}
#endif
	const auto output = iso_8859_1_to_utf8(string(name));
...
#if defined(__ANDROID__)
	if (is_content_uri_path(output.c_str())) {
		mos->fd = android_open_detached_fd_for_uri(output.c_str(), flags);
	} else {
