...
void set_scaling_option(const int monid, const uae_prefs* p, const int width, const int height)
{
	...
	// Android: always scale to the available surface.
	// SDL integer scaling can force a small centered viewport when the best integer
	// factor is 1 (e.g. 800x600 into 1920x1080 => min(2,1)=1), which matches the
	// "800x600 in the middle" symptom and breaks VKBD hit testing.
	integer_scale = SDL_FALSE;
	// Logical-size mode is controlled by a runtime toggle.
	SDL_SetHint(SDL_HINT_RENDER_LOGICAL_SIZE_MODE, stretchToFill ? "stretch" : "letterbox");
	#endif
...
#ifdef AMIBERRY_ANDROID
	// On Android, VKBD is toggled at runtime (overlay button / input code).
	// Allow it regardless of saved preference, and also in RTG/Picasso modes.
	// (RTG sets screen_is_picasso, which would otherwise block VKBD entirely.)
	(void)mon;
	return true;
#else
	if (mon->screen_is_picasso)
...
static bool SDL2_renderframe(const int monid, int mode, int immediate)
{
	...
	{
		static bool s_logged_geometry = false;
		static int s_last_out_w[MAX_AMIGAMONITORS] = { 0 };
		static int s_last_out_h[MAX_AMIGAMONITORS] = { 0 };

		// On some Android devices the surface/renderer output size changes after startup.
		// If the logical-size viewport was computed before the final output size is known,
		// SDL can end up presenting into a stale small viewport ("800x600 centered").
		// Detect output-size changes and reapply logical-size/viewport state.
		if (mon->amiga_renderer)
		{
			int curOutW = 0, curOutH = 0;
			SDL_GetRendererOutputSize(mon->amiga_renderer, &curOutW, &curOutH);
			if (curOutW > 0 && curOutH > 0 &&
				(curOutW != s_last_out_w[monid] || curOutH != s_last_out_h[monid]))
			{
				s_last_out_w[monid] = curOutW;
				s_last_out_h[monid] = curOutH;

				// Force-reset to a known-good full-output viewport before applying logical-size,
				// otherwise SDL may keep computing the logical-size viewport inside a stale
				// smaller viewport.
				SDL_Rect full_vp{ 0, 0, curOutW, curOutH };
				SDL_RenderSetViewport(mon->amiga_renderer, &full_vp);
				SDL_RenderSetClipRect(mon->amiga_renderer, nullptr);
				SDL_RenderSetScale(mon->amiga_renderer, 1.0f, 1.0f);
				SDL_RenderSetIntegerScale(mon->amiga_renderer, SDL_FALSE);

				// Keep logical size consistent with what we actually render.
				if (ad->picasso_on)
				{
					if (amiga_surface->w > 0 && amiga_surface->h > 0)
						SDL_RenderSetLogicalSize(mon->amiga_renderer, amiga_surface->w, amiga_surface->h);
				}
				else
				{
					if (render_quad.w > 0 && render_quad.h > 0)
						SDL_RenderSetLogicalSize(mon->amiga_renderer, render_quad.w, render_quad.h);
				}

				if (vkbd_allowed(monid))
					vkbd_update_position_from_texture();

				// Log geometry again after output size changes.
				s_logged_geometry = false;

				#ifdef AMIBERRY_ANDROID
				__android_log_print(ANDROID_LOG_INFO, "AMIBERRY", "[ANDROID GFX] output changed -> %dx%d (reapplied logical/viewport)", curOutW, curOutH);
				#endif
			}
		}

		if (!s_logged_geometry && mon->amiga_window && mon->amiga_renderer)
		{
			int winW = 0, winH = 0;
			int outW = 0, outH = 0;
			int logW = 0, logH = 0;
			SDL_Rect vp{0, 0, 0, 0};
			SDL_GetWindowSize(mon->amiga_window, &winW, &winH);
			SDL_GetRendererOutputSize(mon->amiga_renderer, &outW, &outH);
			SDL_RenderGetLogicalSize(mon->amiga_renderer, &logW, &logH);
			SDL_RenderGetViewport(mon->amiga_renderer, &vp);
			float scaleX = 0.0f, scaleY = 0.0f;
			SDL_RenderGetScale(mon->amiga_renderer, &scaleX, &scaleY);
			const SDL_bool intScale = SDL_RenderGetIntegerScale(mon->amiga_renderer);
			#ifdef AMIBERRY_ANDROID
			const char* logicalMode = SDL_GetHint(SDL_HINT_RENDER_LOGICAL_SIZE_MODE);
			__android_log_print(ANDROID_LOG_INFO, "AMIBERRY", "[ANDROID GFX] win=%dx%d out=%dx%d logical=%dx%d viewport={%d,%d %dx%d} scale=%.3fx%.3f intScale=%d logicalMode=%s",
				winW, winH, outW, outH, logW, logH, vp.x, vp.y, vp.w, vp.h, scaleX, scaleY, (int)intScale,
				(logicalMode && *logicalMode) ? logicalMode : "(null)");
			__android_log_print(ANDROID_LOG_INFO, "AMIBERRY", "[ANDROID GFX] render_quad={%d,%d %dx%d} crop_rect={%d,%d %dx%d} rotation=%d",
			...
			#else
			write_log("[GFX] win=%dx%d out=%dx%d logical=%dx%d viewport={%d,%d %dx%d} intScale=%d\n",
				winW, winH, outW, outH, logW, logH, vp.x, vp.y, vp.w, vp.h, (int)intScale);
			write_log("[GFX] render_quad={%d,%d %dx%d} crop_rect={%d,%d %dx%d} rotation=%d\n",
				render_quad.x, render_quad.y, render_quad.w, render_quad.h,
				crop_rect.x, crop_rect.y, crop_rect.w, crop_rect.h,
				amiberry_options.rotation_angle);
			#endif
			s_logged_geometry = true;
		}
		...
		if (stretchToFill)
		{
			// Stretch-to-fill: match logical space to the real surface size and draw fullscreen.
			int outW_now = 0, outH_now = 0;
			SDL_GetRendererOutputSize(mon->amiga_renderer, &outW_now, &outH_now);
			if (outW_now > 0 && outH_now > 0)
			{
				int lw_now = 0, lh_now = 0;
				SDL_RenderGetLogicalSize(mon->amiga_renderer, &lw_now, &lh_now);
				if (lw_now != outW_now || lh_now != outH_now)
				{
					SDL_RenderSetLogicalSize(mon->amiga_renderer, outW_now, outH_now);
					// VKBD size/position depends on logical size; refresh immediately.
					vkbd_update_position_from_texture();
					// Re-log geometry once after switching to full-surface logical size.
					s_logged_geometry = false;
				}
			}
		...
		{
			// Aspect-preserving mode: keep the emulator logical size (render_quad) so SDL can
			// letterbox/pillarbox based on SDL_HINT_RENDER_LOGICAL_SIZE_MODE.
			// If we previously stretched, restore the logical size back to the emu dimensions.
			if (render_quad.w > 0 && render_quad.h > 0)
			{
				int lw_now = 0, lh_now = 0;
				SDL_RenderGetLogicalSize(mon->amiga_renderer, &lw_now, &lh_now);
				if (lw_now != render_quad.w || lh_now != render_quad.h)
				{
					SDL_RenderSetLogicalSize(mon->amiga_renderer, render_quad.w, render_quad.h);
					vkbd_update_position_from_texture();
					s_logged_geometry = false;
		...
		if (ad->picasso_on) {
			...
			#ifdef AMIBERRY_ANDROID
			// Respect stretch-to-fill toggle in RTG mode too.
			// Important: if we keep logical size at the RTG surface size, SDL can still
			// end up with a centered viewport on some devices. When stretching, explicitly
			// match logical size to the real output size so we fill the display.
			extern std::atomic<int> g_amiberry_android_stretch_to_fill;
			if (g_amiberry_android_stretch_to_fill.load(std::memory_order_relaxed) != 0)
			{
				int outW_now = 0, outH_now = 0;
				SDL_GetRendererOutputSize(mon->amiga_renderer, &outW_now, &outH_now);
				if (outW_now > 0 && outH_now > 0)
...
{
	#ifdef AMIBERRY_ANDROID
	// Android should always render into the full available surface.
	// Desktop "Windowed" settings can otherwise produce a small centered surface
	// (e.g. 800x600) on some devices.
	return -1; // GFX_FULLWINDOW / SDL_WINDOW_FULLSCREEN_DESKTOP
	#else
...
#ifdef AMIBERRY_ANDROID
	// Android UX: always scale to the full surface. Desktop windowed settings
	// are not meaningful on Android and can lead to a small centered output.
	currprefs.gfx_apmode[APMODE_NATIVE].gfx_fullscreen = changed_prefs.gfx_apmode[APMODE_NATIVE].gfx_fullscreen = GFX_FULLWINDOW;
	currprefs.gfx_apmode[APMODE_RTG].gfx_fullscreen = changed_prefs.gfx_apmode[APMODE_RTG].gfx_fullscreen = GFX_FULLWINDOW;
	mon->currentmode.flags &= ~SDL_WINDOW_FULLSCREEN;
	mon->currentmode.flags |= SDL_WINDOW_FULLSCREEN_DESKTOP;
	fullwindow = 1;
#endif
