...
// Android minimal builds intentionally omit heavy optional audio deps (FLAC/mpg123/etc.).
// Keep CD image support, but compile out FLAC decoding.
#if !defined(AMIBERRY_ANDROID_MINIMAL)
#include "FLAC/stream_decoder.h"
#define AMIBERRY_WITH_FLAC 1
#else
#define AMIBERRY_WITH_FLAC 0
#endif
...
static void flac_get_size (struct cdtoc *t)
{
	// FLAC support not compiled in (e.g. Android minimal build).
	// Leave filesize at 0 so the caller won't enable FLAC track decoding.
	t->filesize = 0;
}
static uae_u8 *flac_get_data (struct cdtoc *t)
{
	// Should not be called unless enctype was set to FLAC, but keep it safe.
	return t->data;
}
#endif
